    ...
    <div>
      <div class="box-top">
        ...
        <div class="input-select">
          <div class="marginSelect">
            按分类筛选：
            <el-select v-model="navId" size="small" class="select-width" placeholder="全部">
              <el-option
      ...
      <div class="box-content">
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column prop="createTime" label="动态发布时间">
            <template slot-scope="scope">
              <p style="cursor: pointer; width:150px;">{{scope.row.createTime}}</p>
            </template>
          </el-table-column>
          <el-table-column prop="navId" label="所属分类">
            <template slot-scope="scope">
              <p style="cursor: pointer">
                {{scope.row.navId == 0 ? '无' : selectList[scope.row.navId].label}}
              </p>
            </template>
          </el-table-column>
          <el-table-column prop="content" label="动态内容">
            <template slot-scope="scope">
              <p class='lineLimit' style="cursor: pointer; ">{{scope.row.content}}</p>
            </template>
          </el-table-column>
...
import { msgList, delMsg } from "../api/index.js";
export default {
  data() {
    return {
      content: "",
  ...
  methods: {
    ...
    detail(row) {
      let messageId = row._id;
      this.$router.push("/message/msgDetail/" + messageId);
    },
    // 编辑
    del(index, row) {
      this.$confirm("确定删除选择动态?", "提示", {
